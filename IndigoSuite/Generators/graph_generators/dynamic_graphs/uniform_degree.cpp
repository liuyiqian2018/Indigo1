...
int main(int argc, char* argv[])
{
  ...
  if ((flag == 0) || (flag == 2)) {
    printf("\nRandom uniform degree graph\n");
    char name1[256];
    sprintf(name1, "uniform_degree_%dn_%de.egr", n, m);
    saveAndPrint(n, m, name1, edges1);

    printf("\nCounter random uniform degree graph\n");
    char name2[256];
    sprintf(name2, "counter_uniform_degree_%dn_%de.egr", n, m);
  ...
  if (filter_nume(m * 2) && ((flag == 0) || (flag == 1))) {
    ...
    char name3[256];
    sprintf(name3, "undirect_uniform_degree%dn_%de.egr", n, m * 2);
