...
int main(int argc, char* argv[])
{
  ...
  if (filter_numv(n)) {
    ...
    if (filter_numv(n) && filter_nume(m1) && ((flag == 0) || (flag == 2))) {
      printf("\nDirected %d-dimensional grid\n", dim);
      char name1[256];
      sprintf(name1, "%ddim_grid_%dn_%de.egr", dim, n, m1);
      saveAndPrint(n, m1, name1, edges1);

      printf("\nCounter %d-dimensional grid\n", dim);
      char name2[256];
      sprintf(name2, "counter%ddim_grid_%dn_%de.egr", dim, n, m1);
    ...
    if (filter_numv(n) && filter_nume(m2 * 2) && ((flag == 0) || (flag == 1))) {
      ...
      char name3[256];
      sprintf(name3, "undirect%ddim_grid_%dn_%de.egr", dim, n, m2 * 2);
